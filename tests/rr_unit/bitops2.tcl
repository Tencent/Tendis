...
start_server {tags {"tbitops"}} {
    ...
    foreach vec [list "" "\xaa" "\x00\x00\xff" "foobar" "123"] {
        ...
        test "BITCOUNT against test vector #$num" {
            r set str $vec
            assert {[r tbitcount str] == [count_bits $vec]}
        }
    }
    ...
    test {BITCOUNT fuzzing without start/end} {
        for {set j 0} {$j < 100} {incr j} {
            ...
            r set str $str
            assert {[r tbitcount str] == [count_bits $str]}
        }
    }
    ...
    test {BITCOUNT fuzzing with start/end} {
        for {set j 0} {$j < 100} {incr j} {
            ...
            assert {[r tbitcount str $start $end] == [count_bits [string range $str $start $end]]}
        }
    }

    test {BITCOUNT with start, end} {
        r set s "foobar"
        assert_equal [r tbitcount s 0 -1] [count_bits "foobar"]
        assert_equal [r tbitcount s 1 -2] [count_bits "ooba"]
        assert_equal [r tbitcount s -2 1] [count_bits ""]
        assert_equal [r tbitcount s 0 1000] [count_bits "foobar"]
    }

    test {BITCOUNT syntax error #1} {
        catch {r tbitcount s 0} e
        set e
    } {ERR*syntax*}
    ...
    test {BITCOUNT regression test for github issue #582} {
        r del foo
        r tsetbit foo 0 1
        if {[catch {r tbitcount foo 0 4294967296} e]} {
    ...
    test {BITOP missing key is considered a stream of zero} {
        r set a "\x01\x02\xff"
        r tbitop and res1 no-suck-key a
        r tbitop or  res2 no-suck-key a no-such-key
        r tbitop xor res3 no-such-key a
        list [r get res1] [r get res2] [r get res3]
    ...
    test {BITOP shorter keys are zero-padded to the key with max length} {
        ...
        r set b "\x01\x02\xff"
        r tbitop and res1 a b
        r tbitop or  res2 a b
        r tbitop xor res3 a b
        list [r get res1] [r get res2] [r get res3]
    ...
    test {BITOP with non string source key} {
        ...
        r set b 2
        r lpush c foo
        catch {r tbitop xor dest a b c d} e
        set e
    } {WRONGTYPE*}
    ...
    test {BITPOS bit=0 unaligned+full word+reminder} {
        ...
        # First zero bit.
        r append str "\x0f"
        assert {[r tbitpos str 0] == 216}
        assert {[r tbitpos str 0 1] == 216}
        assert {[r tbitpos str 0 2] == 216}
        assert {[r tbitpos str 0 3] == 216}
        assert {[r tbitpos str 0 4] == 216}
        assert {[r tbitpos str 0 5] == 216}
        assert {[r tbitpos str 0 6] == 216}
        assert {[r tbitpos str 0 7] == 216}
        assert {[r tbitpos str 0 8] == 216}
    }
    ...
    test {BITPOS bit=1 unaligned+full word+reminder} {
        ...
        # First zero bit.
        r append str "\xf0"
        assert {[r tbitpos str 1] == 216}
        assert {[r tbitpos str 1 1] == 216}
        assert {[r tbitpos str 1 2] == 216}
        assert {[r tbitpos str 1 3] == 216}
        assert {[r tbitpos str 1 4] == 216}
        assert {[r tbitpos str 1 5] == 216}
        assert {[r tbitpos str 1 6] == 216}
        assert {[r tbitpos str 1 7] == 216}
        assert {[r tbitpos str 1 8] == 216}
    }

    test {BITPOS bit=1 returns -1 if string is all 0 bits} {
        r set str ""
        for {set j 0} {$j < 20} {incr j} {
            assert {[r tbitpos str 1] == -1}
            r append str "\x00"
        }
    }
    ...
    test {BITPOS bit=0 works with intervals} {
        ...
        assert {[r tbitpos str 0 0 -1] == 0}
        assert {[r tbitpos str 0 1 -1] == 16}
        assert {[r tbitpos str 0 2 -1] == 16}
        assert {[r tbitpos str 0 2 200] == 16}
        assert {[r tbitpos str 0 1 1] == -1}
    }

    test {BITPOS bit=1 works with intervals} {
        r set str "\x00\xff\x00"
        assert {[r tbitpos str 1 0 -1] == 8}
        assert {[r tbitpos str 1 1 -1] == 8}
        assert {[r tbitpos str 1 2 -1] == -1}
        assert {[r tbitpos str 1 2 200] == -1}
        assert {[r tbitpos str 1 1 1] == 8}
    }

    test {BITPOS bit=0 changes behavior if end is given} {
        r set str "\xff\xff\xff"
        assert {[r tbitpos str 0] == 24}
        assert {[r tbitpos str 0 0] == 24}
        assert {[r tbitpos str 0 0 -1] == -1}
    }
    ...
    test {BITPOS bit=1 fuzzy testing using SETBIT} {
        ...
        for {set j 0} {$j < 1000} {incr j} {
            assert {[r tbitpos str 1] == $first_one_pos}
    ...
    test {BITPOS bit=0 fuzzy testing using SETBIT} {
        ...
        for {set j 0} {$j < 1000} {incr j} {
            assert {[r tbitpos str 0] == $first_zero_pos}
            set pos [randomInt $max]
            r tsetbit str $pos 0
