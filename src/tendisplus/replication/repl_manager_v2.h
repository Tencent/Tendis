#ifndef SRC_TENDISPLUS_REPLICATION_REPL_MANAGER_V2_H_
#define SRC_TENDISPLUS_REPLICATION_REPL_MANAGER_V2_H_
...
class ReplManager {
 public;
    ...
    std::unique_ptr<BlockingTcpClient> ReplManager::createClient(const StoreMeta&);
    ...
    void supplyFullSync(asio::ip::tcp::socket sock, const std::string& storeIdArg);
 ...
 protected:
    void controlRoutine();
    void supplyFullSyncRoutine(std::unique_ptr<BlockingTcpClient> client, uint32_t storeId);
 ...
 private:
    ...
    // slave's pov, master(as client)'s sessionId
    std::vector<std::unique_ptr<uint64_t>> _sessionIds;
...
#endif  // SRC_TENDISPLUS_REPLICATION_REPL_MANAGER_V2_H_
