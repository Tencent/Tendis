...
int main(int argc, char* argv[]) {
  ...
  rocksdb::Status s =
    rocksdb::OptimisticTransactionDB::Open(options, dbname, &txn_db);
  INVARIANT(s.ok());
  PDB pdb(txn_db);
  ...
  for (size_t i = 0; i < thread_num; i++) {
    threads.emplace_back(std::thread(run1,
                                     std::ref(pdb),
                                     key_num,
                                     i,
                                     std::ref(succ_updates),
                                     std::ref(conflict_updates)));
  }
  while (true) {
    sleep(1);
    std::cout << "succ:" << *succ_updates << ",confl:" << *conflict_updates
              << std::endl;
