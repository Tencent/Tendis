...
using CFS =
  std::vector<std::vector<std::unique_ptr<rocksdb::ColumnFamilyHandle>>>;

void usage() {
  std::cout << "./bin [db/odb/pdb] [load/run] db_num cf_num thd_num "
               "memtable_num memtable_size record_num"
            << std::endl;
...
rocksdb::Options _options(std::shared_ptr<rocksdb::Cache> bc,
                          int memtable_num,
                          int memtable_size) {
  ...
  // rocksGlobalOptions.cacheIndexAndFilterBlocks;
  options.table_factory.reset(
    rocksdb::NewBlockBasedTableFactory(table_options));
  options.write_buffer_size = memtable_size;  // 64MB
  options.level0_slowdown_writes_trigger = 8;
  ...
  options.level_compaction_dynamic_level_bytes = true;
  options.max_bytes_for_level_base = 512 * 1024 * 1024;  // 512 MB
  // This means there is no limit on open files. Make sure to always set
  // ulimit so that it can keep all RocksDB files opened.
...
void doPut(const DBS& dbs, const CFS& cfs) {
  ...
  auto s = dbs[dbid]->Put(
    rocksdb::WriteOptions(), cfs[dbid][cfid].get(), rockskey, rocksval);
  assert(s.ok());
}
...
void monitorFunctor(const std::vector<rocksdb::DB*> dbs,
                    const CFS& cfs,
    ...
    for (auto& property : properties) {
      ...
      for (size_t i = 0; i < dbs.size(); i++) {
        for (size_t j = 0; j < cfs[i].size(); j++) {
          dbs[i]->GetIntProperty(
            cfs[i][j].get(), "rocksdb." + property.first, &tmp);
          property.second += tmp;
        }
      }
...
int main(int argc, char* argv[]) {
  ...
  if (db_num == -1 || cf_num == -1 || thd_num == -1 || record_num == -1 ||
      memtable_num == -1 || memtable_size == -1) {
    ...
    if (is_load) {
      if (mode == "db") {
        status = rocksdb::DB::Open(
          _options(bc, memtable_num, memtable_size), s.str(), &db);
      } else if (mode == "pdb") {
        ...
        status = rocksdb::OptimisticTransactionDB::Open(
          _options(bc, memtable_num, memtable_size), s.str(), &odb);
      }
      assert(status.ok());
      for (int j = 0; j < cf_num; j++) {
        ...
        if (mode == "db") {
          status = db->CreateColumnFamily(
            _cfOptions(memtable_num, memtable_size), cf_s.str(), &cfHandler);
        } else if (mode == "pdb") {
          status = pdb->CreateColumnFamily(
            _cfOptions(memtable_num, memtable_size), cf_s.str(), &cfHandler);
        } else if (mode == "odb") {
          status = odb->GetBaseDB()->CreateColumnFamily(
            _cfOptions(memtable_num, memtable_size), cf_s.str(), &cfHandler);
        }
        cfs[i].emplace_back(
          std::unique_ptr<rocksdb::ColumnFamilyHandle>(cfHandler));
        assert(status.ok());
      }
      if (mode == "db") {
        ...
        odbs.emplace_back(
          std::unique_ptr<rocksdb::OptimisticTransactionDB>(odb));
      } else if (mode == "pdb") {
      ...
      status = rocksdb::DB::ListColumnFamilies(
        _options(bc, memtable_num, memtable_size), s.str(), &cfList);
      assert(status.ok());
      std::vector<rocksdb::ColumnFamilyDescriptor> cfDescList;
      std::vector<rocksdb::ColumnFamilyHandle*> handles;
      for (auto& v : cfList) {
        cfDescList.emplace_back(rocksdb::ColumnFamilyDescriptor(
          v, _cfOptions(memtable_num, memtable_size)));
      }
      if (mode == "db") {
        status = rocksdb::DB::Open(_options(bc, memtable_num, memtable_size),
                                   s.str(),
                                   cfDescList,
                                   &handles,
                                   &db);
    ...
    if (mode == "db") {
      if (is_load) {
        threads.emplace_back(std::thread(
          loadFunctor, std::ref(dbs), std::ref(cfs), i, record_num / thd_num));
      } else {
        threads.emplace_back(std::thread(
          runFunctor, std::ref(dbs), std::ref(cfs), i, record_num / thd_num));
      }
    } else if (mode == "odb") {
      if (is_load) {
        threads.emplace_back(std::thread(oloadFunctor,
                                         std::ref(odbs),
                                         std::ref(cfs),
                                         i,
                                         record_num / thd_num));
        ...
        threads.emplace_back(std::thread(
          orunFunctor, std::ref(odbs), std::ref(cfs), i, record_num / thd_num));
      }
    } else {
      if (is_load) {
        threads.emplace_back(std::thread(ploadFunctor,
                                         std::ref(pdbs),
                                         std::ref(cfs),
                                         i,
                                         record_num / thd_num));
        ...
        threads.emplace_back(std::thread(
          prunFunctor, std::ref(pdbs), std::ref(cfs), i, record_num / thd_num));
      }
    }
  }
  ...
  auto monitor =
    std::thread(monitorFunctor, alldbs, std::ref(cfs), std::ref(stopped));
