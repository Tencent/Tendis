...
class RestoreMetaCommand: public Command {
 public:
    ...
    Expected<std::string> run(Session *sess) final {
        ...
        if (!eSlotId.ok()) {
            LOG(ERROR) << "read slot id failed for "
                       << eSlotId.status().toString();
        ...
        while (true) {
            ...
            switch (valueType) {
                ...
                case RecordType::RT_KV:
                    ...
                    break;
                default:
                    LOG(ERROR) << "get invalid record type" << rt2Char(value.getRecordType()) << "in iteration";
                    INVARIANT(0);
            }
